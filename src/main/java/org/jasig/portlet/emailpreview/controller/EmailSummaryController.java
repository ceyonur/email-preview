...
public class EmailSummaryController extends BaseEmailController {
	...
	public enum View {
		...
		WELCOME("welcome") {
			...
			public ModelAndView show(RenderRequest req, RenderResponse res, EmailSummaryController controller) {
				...
				model.put(WELCOME_INSTRUCTIONS_PREFERENCE,
		...
		ROLLUP("rollup") {
			...
			public ModelAndView show(RenderRequest req, RenderResponse res, EmailSummaryController controller) {
				...
				IAuthenticationService authService = controller.authServiceRegistry
		...
		PREVIEW("preview") {
			...
			public ModelAndView show(RenderRequest req, RenderResponse res, EmailSummaryController controller) {
				...
				// PageSize: this value can be set by administrators as a
				// publish-time
				...
				// James W - Removed option for 50 because it has some issues
				// with behavior needing addressing. See EMAILPLT-119
				...
				// Check if this mail server supports setting the READ/UNREAD
				// flag
				model.put(SUPPORTS_TOGGLE_SEEN_KEY, config.supportsToggleSeen());
				// Display message in a table
				model.put(MSG_CONTAINER, "table");

				model.put(MailPreferences.ALLOW_RENDERING_EMAIL_CONTENT.getKey(),
						config.getAllowRenderingEmailContent());

				model.put("inboxName", config.getInboxFolderName());
	...
	public ModelAndView chooseView(RenderRequest req, RenderResponse res) throws Exception {
		...
		if (authService == null || !authService.isConfigured(req, config)) {
			...
			// Rule #3: Use the defaultView preference; this setting gets
			// updated
